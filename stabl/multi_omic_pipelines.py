...
def save_fold_data_subsets(fold_num, omic_name, train_sample_ids, test_sample_ids, 
    ...
    if counts_path and Path(counts_path).exists():
        ...
        gene_cols = counts.columns[:2].tolist()  # First 2 cols = gene info
        
        # Train
        train_present = [c for c in train_sample_ids if c in counts.columns]
        counts[gene_cols + train_present].to_csv(fold_dir / "counts_train.csv", index=False)
        
        # Test
        test_present = [c for c in test_sample_ids if c in counts.columns]
        counts[gene_cols + test_present].to_csv(fold_dir / "counts_test.csv", index=False)
        
        print(f"    ✓ Saved counts: {len(train_present)} train, {len(test_present)} test")
    ...
    if clinical_path and Path(clinical_path).exists():
        ...
        id_col = clinical.columns[0]  # Assume first col = ID
        clinical[id_col] = clinical[id_col].astype(str)
        
        # Train
        train_ids_str = [str(x) for x in train_sample_ids]
        clinical[clinical[id_col].isin(train_ids_str)].to_csv(
            fold_dir / "clinical_train.csv", index=False
        )
        
        # Test
        test_ids_str = [str(x) for x in test_sample_ids]
        clinical[clinical[id_col].isin(test_ids_str)].to_csv(
            fold_dir / "clinical_test.csv", index=False
        )
        
        print(f"    ✓ Saved clinical: {len(train_ids_str)} train, {len(test_ids_str)} test")
...
def multi_omic_stabl_cv(
    ...
    late_fusion=True,
    n_iter_lf=10000,
    original_counts_path=None,      # ← ADD
    original_clinical_path=None,    # ← ADD
):
    """
    ...
    if task_type == "survival" and stabl_cox is not None:
        ...
        if not isinstance(stabl_cox.base_estimator, (CoxPHSurvivalAnalysis, CoxnetSurvivalAnalysis)):
            warnings.warn("For survival, base_estimator should be CoxPHSurvivalAnalysis (Ridge) or CoxnetSurvivalAnalysis (Lasso/ElasticNet)")
    ...
    for train, test in (tbar := tqdm(
        ...
        for omic_name, X_omic in data_dict.items():
            ...
            # Save filtered data files
            if original_counts_path and original_clinical_path:
                save_fold_data_subsets(
                    fold_num=k,
                    omic_name=omic_name,
                    train_sample_ids=X_tmp_std.index.tolist(),
                    test_sample_ids=test_idx_tmp.tolist(),
                    counts_path=original_counts_path,
