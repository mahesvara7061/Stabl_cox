...
def plot_stabl_path(
    ...
    if nb_different_params <= 1:
        if 'alpha' in stabl.fitted_lambda_grid_:
            ...
            # Robust handling for 'alphas' parameter: extract alpha values in the
            # exact order used to compute `stabl_scores_` (i.e. ParameterGrid order),
            # then compute the x positions. This avoids mismatches that create
            # out-of-order connections (vertical jumps).
            ...
            for p in param_grid_all:
                if 'alpha' in p:
                    ...
                    v = p['alphas']
                    # If it's array-like, take the first scalar element
                    ...
                    # no alpha in this param dict: skip
                    continue
            ...
            if alphas_scalar.size <= 1:
                ...
                # Normalize so smaller alpha -> larger x (matching previous behaviour)
        ...
        for i, k in enumerate(params):
            l1_ratio = k.get("l1_ratio", None)
            
            # Robust penalty retrieval
            if "alpha" in k:
                penalty = k["alpha"]
            elif "alphas" in k:
                # Handle list/array value for alphas
                val = k["alphas"]
                if isinstance(val, (list, tuple, np.ndarray)):
                     penalty = float(np.asarray(val).ravel()[0])
                else:
                     penalty = float(val)
            else:
                penalty = k["C"]
        ...
        for l1_ratio in sorted(ordered_params.keys()):
            ...
            if "alpha" in different_params or "alphas" in different_params:
    ...
    try:
        ...
        # If for some reason x values are not strictly positive, skip log scaling
        pass
...
class Stabl(SelectorMixin, BaseEstimator):
    ...
    def _check_lambda_grid(self):
        ...
        if isinstance(self.lambda_grid, str):
            ...
            while not isinstance(base_estimator, (LogisticRegression, Lasso, ElasticNet, CoxnetSurvivalAnalysis)) and hasattr(base_estimator, "model"):
                base_estimator = base_estimator.model
            if not isinstance(base_estimator, (LogisticRegression, Lasso, ElasticNet, CoxnetSurvivalAnalysis)):
                raise ValueError(
                    f'If lambda_grid is "auto", the base_estimator must be a LogisticRegression, '
                    f'Lasso, ElasticNet or CoxnetSurvivalAnalysis, got {base_estimator}'
                )
    ...
    def _get_optimized_lambda_grid(self, X, y):
        ...
        if self.task_type == "survival":
            task_type = "survival"
            if hasattr(self.base_estimator, "l1_ratio"):
                l1_ratio = self.base_estimator.l1_ratio
            else:
                l1_ratio = 1.0
            n_lambda = 100 if self.n_lambda is None else self.n_lambda

        elif isinstance(self.base_estimator, LogisticRegression) or isinstance(getattr(self.base_estimator, "model", None), LogisticRegression):
