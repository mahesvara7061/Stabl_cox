...
def build_stabl_cox(n_bootstraps=500, random_state=42):
    ...
    base = CoxnetSurvivalAnalysis(
        l1_ratio=1.0,       # LASSO Cox; đổi 0.6–1.0 nếu muốn ElasticNet Coxnet
        tol=1e-6,
        max_iter=1_000_000
    )
    ...
    stabl_cox = Stabl(
        ...
        replace=False,
        bootstrap_threshold=1e-5,              # SelectFromModel threshold khi fit mỗi bootstrap
...
def main(args):
    ...
    predictions = multi_omic_stabl_cv(
        ...
        models=models,
        outer_groups=None,
        early_fusion=False,     # có thể bật nếu bạn có nhiều omics
        late_fusion=False,      # survival late-fusion đã được vá; bật khi multi-omic
        n_iter_lf=10000
    )
